...
const getCharIndex = (info, c) => {
  ...
  if (
    (info[BYTES_PER_CHAR * min] << 8) + info[BYTES_PER_CHAR * min + 1] > c ||
    (info[BYTES_PER_CHAR * max] << 8) + info[BYTES_PER_CHAR * max + 1] < c
  ) {
    return -1;
  }
  ...
  this.redraw = () => {
    ...
    switch (this._valign) {
      case "top":
        ...
        break;
      case "middle":
        offy -= (this._info[INDEX_ASCENT] + this._info[INDEX_DESCENT]) / 2;
        break;
      case "bottom":
        offy -= this._info[INDEX_ASCENT] + this._info[INDEX_DESCENT];
        break;
      case "baseline":
      default:
        offy -= this._info[INDEX_ASCENT];
        break;
    }
